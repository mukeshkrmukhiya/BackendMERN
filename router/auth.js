...
//     return res.status(400).json({ message: "phone must be a number", msg:  typeof phone })
//    }
    ...
    try {
      ...
      const ClientToken = jwt.sign(data, process.env.SECRET_KEY);
      success = true;
    //   res.json({success, ClientToken })


        if (userResgiter) {
            res.status(201).json({success, ClientToken, message: "Resitration succesfull" })
        } else {
    ...
    try {
        ...
        if (!userLogin) {
            ...
            if (isMatch) {
                ...
                const ClientToken = jwt.sign(data, process.env.SECRET_KEY);
                success = true;
                res.status(200).json({success,ClientToken , message: "signin successful" })
